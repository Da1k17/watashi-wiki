...
## 直感で答えたこと
## AIへのメモ
「直感で答えたこと」は、3秒で答えた直感チェックの結果（そう思う／ちがう／パス、平均秒数）を1〜2文でそのまま要約する。無ければ「（未実施）」。
「AIへのメモ」は、この人にAIが答えるときの方針を3行の箇条書き（言葉づかい、前提にしてよい状況、避けること）。
JSON以外の文字は出力しない。`;
...
function templateWiki(name, answers) {
  ...
  L.push("## AIへのメモ", "- 専門用語を使わず、短く答える。［共通］",
    j("q4") ? `- 前提: ${j("q4")}。［Q4］` : "- 前提: 年代・暮らしは未回答。［Q4］",
    j("q3") ? `- ${j("q3")}が気になっているので、その視点を先に。［Q3］` : "- 気になっていることは未回答。［Q3］");
  return L.join("\n");
}
